  ...
  (function(Range17) {
    function create(one, two, three, four) {
      if (Is.number(one) && Is.number(two) && Is.number(three) && Is.number(four)) {
        return {start: Position9.create(one, two), end: Position9.create(three, four)};
    ...
    function is2(value) {
      var candidate = value;
      return Is.objectLiteral(candidate) && Position9.is(candidate.start) && Position9.is(candidate.end);
    }
    Range17.is = is2;
  })(Range16 || (Range16 = {}));
  var Location4;
...
var Position;
(function(Position9) {
  Position9.fromLocation = (tslocation) => {
    return {
      line: tslocation.line - 1,
      character: tslocation.offset - 1
    };
  };
  Position9.toLocation = (position) => ({
    line: position.line + 1,
    offset: position.character + 1
  });
  Position9.toFileLocationRequestArgs = (file2, position) => ({
    file: file2,
    line: position.line + 1,
    offset: position.character + 1
  });
...
function convertCompletionEntry(tsEntry, uri, position, context) {
  ...
  if (tsEntry.isImportStatementCompletion) {
    insertText = label;
    insertTextFormat = import_vscode_languageserver_protocol6.InsertTextFormat.Snippet;
  }
  let textEdit = null;
...
var FileReferencesCommand2 = class {
  ...
  async execute() {
    ...
    if (client.serviceClient.apiVersion.lt(FileReferencesCommand2.minVersion)) {
      import_coc12.window.showMessage("Find file references failed. Requires TypeScript 4.2+.", "error");
      return;
    }
    ...
    if (!openedFiledPath)
      return;
    const response = await client.serviceClient.execute("fileReferences", {file: openedFiledPath}, import_coc12.CancellationToken.None);
    if (response.type !== "response" || !response.body)
      return;
    const locations = response.body.refs.map((r) => Location2.fromTextSpan(client.serviceClient.toResource(r.file), r));
    await import_coc12.commands.executeCommand("editor.action.showReferences", doc.uri, import_vscode_languageserver_types2.Position.create(0, 0), locations);
  }
};
