...
var reloadImportRegistries = new import_coc.RequestType0("deno/reloadImportRegistries");
var registryState = new import_coc.NotificationType("deno/registryState");
...
async function test(uri, name) {
  ...
  terminal = await import_coc2.workspace.createTerminal({ name, cwd: import_coc2.workspace.root });
  terminal.sendText(`${bin} ${args.join(" ")}`);
}
function reloadImportRegistries2(_context, client2) {
  return () => client2.sendRequest(reloadImportRegistries);
}
...
async function activate(context) {
  ...
  const run = {
    command,
    args: ["lsp"],
    options: { env: __spreadProps(__spreadValues({}, process.env), { "NO_COLOR": true }) }
  };
  ...
  if (semver.valid(serverVersion) && !semver.satisfies(serverVersion, SERVER_SEMVER)) {
    import_coc3.window.showMessage(`The version of Deno ("${serverVersion}") does not meet the requirements of version ("${SERVER_SEMVER}"), please upgrade Deno.`, "warning");
  }
}
function deactivate() {
...
function createRegistryStateHandler() {
  return async function handler(p) {
    let enable = false;
    if (p.suggestions) {
      const selection = await import_coc3.window.showInformationMessage(`The server "${p.origin}" supports completion suggestions for imports. Do you wish to enable this? (Only do this if you trust "${p.origin}") [Learn More](https://github.com/denoland/vscode_deno/blob/main/docs/ImportCompletions.md)`, "No", "Enable");
